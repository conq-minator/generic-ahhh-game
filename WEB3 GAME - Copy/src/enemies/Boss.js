...
export class Boss {
  constructor(scene, config) {
    ...
    // Model: Default placeholder, gets overridden by subclasses
    this.model = new THREE.Group(); 
    this.model.add(new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshBasicMaterial({color: 0xff0000})));
  ...
  takeDamage(amount) {
    ...
    // Flash (only on materials that have emissive; MeshBasicMaterial has no emissive and would throw)
    this.model.traverse(c => {
      ...
      materials.forEach((mat, i) => {
        try {
          ...
          if (Array.isArray(c.material)) c.material[i] = m;
          else c.material = m;
          setTimeout(() => {
            try {
              if (m.emissive) {
        ...
        } catch (_) { /* skip materials without emissive */ }
      });
    });
...
export class DefenseBoss extends Boss {
  constructor(scene, position) {
    super(scene, {
      ...
      gearDrop: {
        type: 'Chestplate',
        name: 'Titan Guard Armor',
        rarity: 'Legendary',
        description: 'A massive chestplate dropped from the Iron Guardian.',
        attributes: [
          { trait_type: 'Item Type', value: 'Chestplate' },
    ...
    // Replace inherited simple model with specialized terrifying model
    this.scene.remove(this.model);
    this.model = AssetManager.createDefenseBossModel();
    this.model.visible = false;
    this.scene.add(this.model);
  }
}
  ...
  constructor(scene, position) {
    super(scene, {
      ...
      gearDrop: {
        ...
        rarity: 'Legendary',
        description: 'Ethereal boots dropped from the Shadow Striker.',
        attributes: [
          { trait_type: 'Item Type', value: 'Boots' },
    ...
    // Replace inherited simple model with specialized terrifying model
    this.scene.remove(this.model);
    this.model = AssetManager.createSpeedBossModel();
    this.model.visible = false;
    this.scene.add(this.model);
  }
  ...
  constructor(scene, position) {
    super(scene, {
      ...
      gearDrop: {
        ...
        name: 'Inferno Fang',
        rarity: 'Legendary',
        description: 'A legendary sword dropped from the Dark Overlord.',
        attributes: [
    ...
    // Replace inherited simple model with specialized terrifying model
    this.scene.remove(this.model);
    this.model = AssetManager.createAttackBossModel();
    this.model.visible = false;
    this.scene.add(this.model);
  }
}
